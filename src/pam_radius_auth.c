...
static void build_radius_packet(AUTH_HDR *request, CONST char *user, CONST char *password, radius_conf_t *conf)
{
	...
	/* Add custom attribute "Connection_Info 77". ADV.ru. Start */
	const char *client_host = getenv("common_name");
	const char *client_ip   = getenv("untrusted_ip");

	if (client_host || client_ip) {
    char connect_info[256];
    snprintf(connect_info, sizeof(connect_info),
             "Device %s. IP %s",
             client_host ? client_host : "unknown",
             client_ip ? client_ip : "unknown");

    syslog(LOG_INFO, "Adding Connect-Info: %s", connect_info);

    add_attribute(request, PW_CONNECT_INFO,
                  (unsigned char *)connect_info,
                  strlen(connect_info));
	}
	
	/* Add custom attribute "Connection_Info 77". ADV.ru. Finish */
